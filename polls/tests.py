...
class ChoiceModelTest(TestCase):
    ...
    def test_creating_some_choices_for_a_poll(self):
        ...
        #try retrieving it from the database, using the poll objcet's reverse
        # lookup
        poll_choices = poll.choice_set.all()
        self.assertEquals(poll_choices.count(), 1)

        # finally, check its attibutes have been saved
...
class SinglePollViewTest(TestCase):
    ...
    def test_page_shows_title_and_no_votes_message(self):
        # set up two polls, to check the right is displayed
        poll1 = Poll(question='6 times 7', pub_date=timezone.now())
        poll1.save()
        poll2 = Poll(question='life, the universe and everything', pub_date=timezone.now())
        poll2.save()
        ...
        #check the poll's question appears on the pages
        self.assertIn(poll2.question, response.content)

        # check our 'no votes yet' message appers
...
class PollsVoteFormTest(TestCase):
    ...
    def test_form_renders_poll_choices_as_radio_inputs(self):
        ...
        #set up another poll to make sure we only see the rigth choices
        poll2 = Poll(question='time', pub_date=timezone.now())
        poll2.save()
        choice3 = Choice(poll=poll2, choice='PM', votes=0)
        choice3.save()

        # build a voting form for poll
        form = PollVoteForm(poll = poll1)

        # check is has a single field called 'vote', which has right choices
        ...
        # check is uses inputs to render
